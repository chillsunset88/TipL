...
export default function TripsScreen() {
  ...
  const s = React.useMemo(() => StyleSheet.create({
    ...
    countryPill: {
      ...
      backgroundColor: isDark ? 'rgba(197,162,103,0.12)' : 'rgba(197,162,103,0.22)',
      paddingHorizontal: 6,
      paddingVertical: 3,
    ...
    cardOwn: { borderColor: C.primary, borderWidth: 2, backgroundColor: isDark ? '#1C150A' : '#FFFDF7' },
    ...
    capBadgeOwn: { backgroundColor: C.primary, borderColor: C.primary },
    capTxt: {
      fontFamily: Typography.regular.fontFamily,
      fontSize: 11,
      color: C.primary,
    ...
    routeRowOwn: { backgroundColor: isDark ? '#281E10' : '#FFF8EC' },
    ...
    routeLine: { flex: 1, height: 1.5, backgroundColor: C.midGray },
    routeLineOwn: { backgroundColor: C.primaryLight },
    planeWrap: {
      width: 28,
      height: 28,
      borderRadius: 14,
    ...
    planeWrapOwn: { backgroundColor: C.primary },
      ...
      >
        ...
        {myTrips.length > 0 && (
          <View style={s.section}>
            ...
            {myTrips.map((trip) => (
              <TripCard key={trip.id} trip={trip} isOwn s={s} C={C} />
            ))}
          </View>
        )}
        ...
        <View style={s.section}>
          ...
          {tripsLoading ? (
            <ActivityIndicator color={C.primary} style={{ marginVertical: Spacing.base }} />
          ) : upcomingTrips.length === 0 ? (
            <View style={s.empty}>
              <Ionicons name="airplane-outline" size={40} color={C.midGray} />
            ...
            upcomingTrips.map((trip) => <TripCard key={trip.id} trip={trip} isOwn={false} s={s} C={C} />)
          )}
        </View>
      </ScrollView>
    </SafeAreaView>
...
// ─── Country Card ─────────────────────────────────────────────────────────────
function CountryCard({ country, onPress, s }: {
      ...
      <LinearGradient colors={['transparent', 'rgba(20,10,2,0.88)']} style={StyleSheet.absoluteFill}>
        <View style={s.countryInfo}>
          ...
          <View style={s.countryPill}>
            <Ionicons name="location-outline" size={9} color={s.countryPillTxt.color} />
...
// ─── Trip Card ─────────────────────────────────────────────────────────────────
function TripCard({ trip, isOwn, s, C }: {
      ...
      {isOwn && (
        <View style={s.ownBanner}>
          ...
          <TouchableOpacity style={s.manageBtn} onPress={() => router.push(`/trip/${trip.id}`)}>
            ...
            <Ionicons name="settings-outline" size={12} color={C.primary} />
          </TouchableOpacity>
        </View>
      )}
      ...
      <View style={s.cardTop}>
        ...
        {trip.capacity_kg != null && (
          <View style={[s.capBadge, isOwn && s.capBadgeOwn]}>
            <Text style={[s.capTxt, isOwn && { color: '#FFFFFF' }]}>{trip.capacity_kg} kg</Text>
          </View>
        )}
      </View>
      ...
      <View style={[s.routeRow, isOwn && s.routeRowOwn]}>
        ...
        <View style={s.routeArrow}>
          ...
          <View style={[s.planeWrap, isOwn && s.planeWrapOwn]}>
            <Ionicons name="airplane" size={14} color={isOwn ? '#FFFFFF' : C.primary} />
          </View>
          <View style={[s.routeLine, isOwn && s.routeLineOwn]} />
        </View>
