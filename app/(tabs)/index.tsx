...
function TravelerCard({ trip }: { trip: TripWithProfile }) {
  ...
  // ── Destructure dari profiles ──────────────────────────────────────────────
  const travelerName = trip.profiles?.full_name ?? 'Traveler';
  const travelerAvatar = trip.profiles?.avatar_url ?? null;
  const travelerRating = trip.profiles?.rating ?? 0;
  const travelerTotalTrips = trip.profiles?.total_trips ?? 0;
  ...
  const handleChat = async () => {
    ...
    if (user.id === trip.triper_id) {          // ← travelerId → triper_id
      Alert.alert('Info', 'This is your own trip.');
      return;
    }
    setChatLoading(true);
    try {
      const roomId = await getOrCreateChatRoom(user.id, trip.triper_id, null);  // ← sama
  ...
  return (
    <TouchableOpacity style={s.tripCard} activeOpacity={0.7} onPress={() => router.push(`/trip/${trip.id}`)}>
      <View style={s.tripRow}>
        <Avatar uri={travelerAvatar} name={travelerName} size="md" />   {/* ← fix */}
        <View style={s.tripInfo}>
          <Text style={s.tripName}>{travelerName}</Text>                {/* ← fix */}
          <View style={s.ratingRow}>
            <Ionicons name="star" size={12} color={Colors.primary} />
            <Text style={s.ratingTxt}>
              {travelerRating > 0 ? travelerRating.toFixed(1) : '—'}    {/* ← fix */}
            </Text>
            <Text style={s.reviewCnt}>· {travelerTotalTrips} trips</Text> {/* ← fix */}
          </View>
        </View>
