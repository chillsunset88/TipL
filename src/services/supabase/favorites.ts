...
export async function isFavoriteTriper(userId: string, triperId: string): Promise<boolean> {
  const { data } = await (supabase.from('favorite_tripers') as any)
    .select('id')
    .eq('user_id', userId)
...
export async function toggleFavoriteTriper(userId: string, triperId: string): Promise<boolean> {
  ...
  if (already) {
    await (supabase.from('favorite_tripers') as any).delete().eq('user_id', userId).eq('triper_id', triperId);
    return false;
  } else {
    await (supabase.from('favorite_tripers') as any).insert({ user_id: userId, triper_id: triperId });
    return true;
  }
}

export async function getFavoriteTripers(userId: string) {
  const { data, error } = await (supabase.from('favorite_tripers') as any)
